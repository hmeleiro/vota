...
Cuando una funcion falla dentro de
[`purrr::map()`](https://purrr.tidyverse.org/reference/map.html), el
mensaje de error original puede perderse o ser confuso para el usuario
final.
...
Version segura de
[`purrr::map()`](https://purrr.tidyverse.org/reference/map.html) que
captura errores y proporciona contexto sobre que elemento fallo.

#### `safe_map_dfr()`

Version segura de
[`purrr::map_dfr()`](https://purrr.tidyverse.org/reference/map_dfr.html)
que combina resultados en data.frame y proporciona manejo robusto de
errores.
